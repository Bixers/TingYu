...
### v0.1.9 (2026-04-28)
- 修复每日雨丝推送在微信接口返回 `text/plain;charset=utf-8` 时的解析失败问题
- 每日雨丝改为先接收原始响应字符串，再由后端统一解析 JSON，兼容获取 `access_token` 与发送订阅消息两条链路
- 补充更明确的微信返回错误提示，便于后续定位 `errcode` / `errmsg`

### v0.1.8 (2026-04-24)
- 缩紧“每日雨丝”卡片布局，按钮改为居中窄版面板，降低“我的”页首屏压迫感
- 雨声播放增加应用级收口，退出页面或切后台时会强制停止并销毁音频实例
- 保持阅读足迹、收藏与摘录功能不变，继续聚焦合规的内容阅读能力
