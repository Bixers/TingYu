...
CREATE TABLE IF NOT EXISTS favorites (
    ...
    favorite_type VARCHAR(20) NOT NULL COMMENT '收藏类型：FULL=全文，SENTENCE=句子',
    poem_id VARCHAR(64) NOT NULL COMMENT '诗词ID',
    target_key VARCHAR(255) NOT NULL COMMENT '收藏唯一键',
    sentence_index INT DEFAULT NULL COMMENT '句子序号',
    sentence_text TEXT COMMENT '句子内容',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    INDEX idx_user_id (user_id),
    INDEX idx_poem_id (poem_id),
    INDEX idx_favorite_type (favorite_type),
    UNIQUE KEY uk_user_type_target (user_id, favorite_type, target_key)
